...
def create_total_mass_by_locality_method_chart():
    ...
    if management_data.empty or locality_data.empty:
        return go.Figure().update_layout(
            title="Sem dados de manejo disponíveis",
            height=450
        )

    # Filter to REBIO + Entorno and sanitize core numeric field
    management_data = management_data[management_data['locality_id'].isin(REBIO_ENTORNO_LOCALITIES)].copy()
    ...
    if management_data.empty:
        return go.Figure().update_layout(
            title="Sem dados válidos de manejo para REBIO + Entorno",
            height=450
        )
    ...
    management_data['name'] = management_data['name'].fillna(
...
def get_report_layout():
    ...
    layout = dbc.Container([
        ...
        # Total Mass by Locality and Method Section
        html.Hr(className="mt-5"),
        ...
        dcc.Loading(
            dcc.Graph(
                id='report-total-mass-locality-method-chart',
                figure=create_total_mass_by_locality_method_chart()
            ),
            type="circle"
        ),
        ...
        html.Button("Voltar", id="back-to-dashboard-report-btn", n_clicks=0, style={"margin-top": "30px"}),
